...
filepath_list = [
    file_path 
    for file_path in (base_dir.parent / DATA_DIR_NAME / DATA_CLEANED_SUBDIR_NAME).glob('*.parquet')
...
for filepath in filepath_list:
    ...
    source_year_month_str = str(filepath).split('/')[-1].split('_')[-1].rstrip('.parquet').strip()
...
max_source_plus_one_month_datetime = ride_ts_df["source_year_month"].max() + MonthBegin(1)
...
zero_filled_ts_df = (
    pd.DataFrame(
        pd.date_range(start = ride_ts_df["pickup_date"].min(), end = ride_ts_df["pickup_date"].max(), freq="D")
    )
...
# %%
ride_ts_df = ride_ts_df[ride_ts_df["Borough"].isin(["Manhattan"])].reset_index(drop=True)
...
for year in ride_ts_df["pickup_date"].dt.year.unique():
    ...
    temp_pivot_df = temp_df.pivot_table(index="PULocationID", columns="pickup_date", values="count")
...
data = {'date': pd.to_datetime(['2024-01-01', '2024-01-01', '2024-01-02', '2024-01-02']),
            'hour': [9, 14, 10, 15],
            'value': [25, 30, 28, 32]}
