...
for file_path in (base_dir.parent / DATA_DIR_NAME / DATA_SUBDIR_NAME).glob("*.parquet"):
    ...
    file_month_start_datetime = pd.to_datetime(
        filename.rstrip(".parquet").split("_")[2]
    )
    ...
    taxi_df = taxi_df[
        ~((taxi_df["trip_distance"] == 0) & (taxi_df["total_amount"] <= 0))
    ]
    taxi_df.to_parquet(
        base_dir.parent / DATA_DIR_NAME / DATA_CLEANED_SUBDIR_NAME / output_filename
    )
    ...
    taxi_agg_df.to_parquet(
        base_dir.parent
        / DATA_DIR_NAME
        / DATA_CLEANED_SUBDIR_NAME
        / output_aggregated_filename
    )
