...
taxi_ride_filepath = (
    base_dir.parent / DATA_DIR_NAME / DATA_SUBDIR_NAME / TAXI_RIDE_FILENAME
)
...
# %%
file_month_start_datetime = pd.to_datetime(
    TAXI_RIDE_FILENAME.rstrip(".parquet").split("_")[2]
)
...
null_count_per_vendor = (
    taxi_df[groupby_col]
    .merge(null_mask, left_index=True, right_index=True)
    .groupby(groupby_col)
    .sum()
)
...
# %%
taxi_df["difference_minutes_pickup_month_start"] = (
    taxi_df["tpep_pickup_datetime"] - file_month_start_datetime
).dt.total_seconds() / 60

# %%
taxi_df["difference_minutes_pickup_month_next_start"] = (
    taxi_df["tpep_pickup_datetime"] - file_next_month_start_datetime
).dt.total_seconds() / 60
...
temp_mask = (taxi_df["tpep_pickup_datetime"] < start_cutoff_pickup) | (
    taxi_df["tpep_pickup_datetime"] > end_cutoff_pickup
)
...
taxi_df[
    ...
    & ((taxi_df["fare_amount"] == 0) | (taxi_df["total_amount"] == 0))
]
...
# %%
taxi_agg_df = (
    taxi_df[["pickup_date", "PULocationID", "count"]]
