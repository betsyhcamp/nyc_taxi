...
def prepare_sql(sql_path: Path, sql_params: Mapping[str, object]) -> PreparedSql:
    """Read SQL, render Jinja placeholders, and return text plus a SHA-256 digest.

    Args:
        sql_path: A .sql file, optionally containing Jinja placeholders.
...
def build_run_scoped_uri(bucket: str, prefix: str, run_id: str, filename: str) -> str:
    ...
    Args:
        ...
        filename: Final path segment, e.g. "panel.parquet".
    """
    return f"gs://{bucket}/{prefix}/{run_id}/{filename}"


def require_gcs_uri(gcs_uri: str) -> None:
...
def write_text_to_gcs(text: str, gcs_uri: str) -> None:
    """Write text to a GCS object as utf-8.

    A candidate for promotion to tsbricks once the API stabilizes.

    Args:
        text: The string to write.
        gcs_uri: Destination object URI, e.g. "gs://bucket/dev/query.sql".

    Raises:
        ValueError: gcs_uri is not a gs:// URI. Unguarded, a bare path or an
            s3:// URI resolves to another backend and the write *succeeds*.
    """
    require_gcs_uri(gcs_uri)
    fs, path = fsspec.url_to_fs(gcs_uri)
    # Named rather than inherited: fsspec otherwise wraps at the platform
    # default, making "utf-8" true by environment rather than by contract.
    with fs.open(path, mode="w", encoding="utf-8") as f:
        f.write(text)
...
def download_from_gcs(gcs_uri: str, destination_dir: Path) -> Path:
    ...
    Half of the local substitute for the gcsfuse mount: pathlib.Path is no
    abstraction over remote storage, so the local runner stages bytes on disk to
    make the identical call Vertex makes through its mount.
    ...
    Raises:
        ValueError: gcs_uri is not a gs:// URI, or names a prefix rather than an
            object. Directory support needs listing and path reconstruction.
    """
    require_gcs_uri(gcs_uri)
...
def upload_to_gcs(local_path: Path, gcs_uri: str) -> int:
    ...
    The other half of the gcsfuse substitute; see download_from_gcs.

    A directory's *contents* land under gcs_uri, its basename dropped. The
    trailing "/" appended to the source is what guarantees that, since fsspec
    appends the basename otherwise. Existing objects are overwritten and parent
    prefixes are implicit.

    Args:
        local_path: File or directory to upload, recursively for a directory.
        gcs_uri: Destination prefix ending in "/", e.g.
            "gs://bucket/dev/train/RUNID/compose_configs/".

    Returns:
        Objects written: 1 for a file, the file count for a directory.
...
def sync_to_gcs(
    ...
    Args:
        local_dir: Directory to publish. Its contents land under gcs_uri, its
            basename dropped.
        gcs_uri: Destination prefix ending in "/", e.g.
            "gs://bucket/dev/train/RUNID/compose_configs/".
        completion_marker: File in local_dir whose presence means the prefix is
            complete. None publishes no marker.
    ...
    Raises:
        ValueError: gcs_uri is not a gs:// prefix ending in "/", or
            ...
            key it differently from every body object.
    ...
    """
    # Ahead of every remote call: this deletes before it writes, and a bare
    # local path resolves to LocalFileSystem.
    _require_gcs_prefix(gcs_uri)
    ...
    # verbatim, so nesting needs no path arithmetic and no basename is added.
    fs.put(
        [str(p) for p in body],
    ...
    if marker is not None:
        ...
        # marker sent with the body can land while a sibling is missing.
